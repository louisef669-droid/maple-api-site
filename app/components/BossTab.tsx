...
const bossGroups = [
  ...
  {
    name: "노진힐라인",
    bosses: ["하드 스우", "하드 데미안", "노말 루시드", "노말 윌", "노말 듄켈", "노말 진힐라"],
  },
  {
    name: "하드 검밑솔",
...
export default function BossTab({
  ...
  return (
    <div>
      ...
      <p style={{ color: "#aaa" }}>
        체크됨 : {checkedCardCount} / {bossCards.length}
      </p>
      ...
      >
        {bossCards.map((card) => {
          const checkedBoss = card.variants.find((boss) =>
            checkedBosses.includes(boss.name)
          );

          const selectedBossName =
            checkedBoss?.name ??
            selectedBossByBase[card.baseName] ??
            card.variants[0].name;

          const selectedBoss =
            card.variants.find((boss) => boss.name === selectedBossName) ??
            card.variants[0];
